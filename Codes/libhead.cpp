...
template<class ForwardIterator, class T> void iota(ForwardIterator first, ForwardIterator last, int value)
{
    while(first != last)
    {
        *first++ = value;
...
{
    vi temp(popSize);
    iota<std::vector<int, std::allocator<int> >::iterator,int>(temp.begin(),temp.end(),0);
