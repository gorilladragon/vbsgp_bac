...
void SGPPlus::approx(vd &_mu, vd &_sigma, vd &_Kmm_inv)
{
    ...
    // RETURN MU & SIGMA
    _mu.clear();
    SFOR(i, data->nSupport) _mu.push_back(SM->second(i, 0));

    cout << "Mu size " << _mu.size() << endl;

    // SIGMA is returned in 1-D vector form, you can unpack them later in MATLAB
    _sigma.clear();
    NFOR(i, j, data->nSupport, data->nSupport) _sigma.push_back(SM->first(i, j));
    cout << "Sigma size " << _sigma.size() << endl;
    
    _Kmm_inv.clear();
    NFOR(i, j, data->nSupport, data->nSupport) _Kmm_inv.push_back(Kmm_inv(i, j));
    cout << "Kmm_inv size " << _Kmm_inv.size() << endl;
}
...
Prediction* SGPPlus::predict(mat &qfm,mat &sigma)
{
    ...
    Prediction* result  = new Prediction(pred);
